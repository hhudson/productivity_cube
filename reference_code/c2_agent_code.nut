...
function postToBS(data,id,impSSID) {
    

    // insert the schema retrieved above into the request url
	local url = "https://apex.oracle.com/pls/apex/anton/bshDevice/info/2/10001/";
	//local headers = { "User-Agent" : "C2-Imp-Lib/0.1" };
	
	local headers = { "Content-Type": "application/json" };
	
	local requestData = { "a1": id, "n1": data, "a2": id, "n2": data };
	local body = http.jsonencode(requestData);
	...
	if(response.statuscode != 201) {
	    server.log("bs error with http request, status: " + response.statuscode);
		server.log("bs error with http request, body: " + response.body);
    ...
    server.log("Posted to BS: "+data+", got return code: "+response.statuscode+", msg: "+response.body) ;
...
device.on("data", function(datapoint) {
    postToBS(datapoint.temp, datapoint.id, datapoint.impSSID);
